...
#define SLAVEOUTDIR "inputdir.out.0"
#else   
...
// Test ExecuteCommand (ie create slave output and tmp directory, execute the command on a block - NO Tmp
// AN ERROR WILL BE GENERATED BY THE EXTERNAL COMMAND
TEST_F(ChdbTest1,ExecuteCommandNoTmp) {
	...
	EXPECT_EQ(outputdir,dir.getOutDir());
	...
	// executeCommand will create the output directory "as if" I were a slave
	EXPECT_NO_THROW(sched.executeCommand());
	ofstream err;
	EXPECT_EQ(true,sched.errorHandle(err));
	outputdir = SLAVEOUTDIR;
	EXPECT_EQ(expected_file_contents["B.txt"],readFile(outputdir+"/B.txt"));
...
TEST_F(ChdbTest1,ExecuteCommandFrmList1) {
	...
	// Remove errors.txt
	system("rm -r errors.txt inputdir.out*");
...
TEST_F(ChdbTest1,ExecuteCommandFrmList2) {
	...
	// Remove files.txt
	system("rm -r files.txt inputdir.out*");
